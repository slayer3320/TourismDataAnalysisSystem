...
class spider(object):
    def __init__(self):
        self.url = 'https://piao.qunar.com/ticket/list.json?keyword=%s&page=%s' #删除掉地区和页数
        self.detailUrl = 'https://piao.qunar.com/ticket/detail_%s.html' #删除sightID
    ...
    def spiderMain(self,resp,province):
        respJSON = resp.json()['data']['sightList'] #将response对象数据直接转换成json数据，respJSON是字典类型的数据
        for index,travel in enumerate(respJSON):  #枚举 遍历
            ...
            respDetailXpath = etree.HTML(self.send_request(detailUrl).text)  #lxml库的etree,方便从HTML源码中获取内容,解析字符串格式的HTML文档对象，将字符串转变为_element对象
            score = respDetailXpath.xpath('//span[@id="mp-description-commentscore"]/span/text()')  #xpath解析对象数据，定位提取所需数据
            if not score:
                score = 0
                star = 0
            else:
            ...
            commentsTotal = respDetailXpath.xpath('//span[@class="mp-description-commentCount"]/a/text()')[0].replace('条评论','') #将文字'条评论'替换为空值
            detailIntro = respDetailXpath.xpath('//div[@class="mp-charact-intro"]//p/text()')[0]
            img_list = respDetailXpath.xpath('//div[@class="mp-description-image"]/img/@src')[:6]  #取DOM元素的属性，用@
            # ================================= 评论爬取
            ...
            try:
                ...
                for c in commentsList:
                    if c['content'] != '用户未点评，系统默认好评。':
                        ...
                        date = c['date']
                        score = c['score']
                        comments.append({     #comments列表添加元素
                            'author': author,
                            'content': content,
                            'date': date,
                            'score': score
            ...
            resultData = []   #数据就位后，添加进resultData列表
            ...
            resultData.append(json.dumps(comments))  #将python数据结构转换成JSON，即dict类型转换为str类型
        ...
        with open('./city.csv','r',encoding='utf8') as readerFile:
            ...
            for cityData in readerCsv:
                for page in range(1,2):
                    try:
                        url = self.url % (cityData[0], page)  #重定义url，拼接上数据
